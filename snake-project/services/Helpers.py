...
class Helpers:
    ...
    def cornerRect(img, bbox, l=30, t=5, rt=1,
                colorR=(255, 0, 255), colorC=(238, 194, 255)):
        """
    ...
    def putTextRect(img, text, pos, scale=3, thickness=3, colorT=(255, 255, 255),
                    colorR=(109, 67, 126), font=cv2.FONT_HERSHEY_PLAIN,
...
def main():
    ...
    while True:
        img = cap.read()
        img = Helpers.putTextRect(img, "CVZone", [50, 50], 2, 2, offset=10, border=5)
        imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        imgList = [img, img, imgGray, img, imgGray]
        imgStacked = Helpers.stackImages(imgList, 2, 0.5)
